...
public class PlayerController_BossBT : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.tag == "Enemy-Animal" && !isDeath && !isHurt)   //敵人設計
        {
    ...
    public void BossDead(int BOSSCode)
    {       
        BossDeadCode  = BOSSCode;
        Invoke("Nextscene", 5f);
    }
    ...
    {
        if (isdefense && !isDeath)
        {
            currentHealth -= damage / 2;
            //hurtAudio.Play();
        }
        else if (!isdefense && !isDeath)
        {
